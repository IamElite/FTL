...
from src.bot.clients import cleanup_clients, initialize_clients
...
PLUGIN_PATH = "src/bot/plugins/*.py"
VERSION = __version__


def print_banner():
    banner = f"File Streaming Bot v{VERSION}"
    print(banner)
...
async def import_plugins():
    ...
    for file_path in plugins:
        try:
            ...
            import_path = f"src.bot.plugins.{plugin_name}"
